...
class FileTracker {
    ...
    async trackFolder(folderPath) {
        let filePaths = await file_1.getFilePathsMathGlobPattern(folderPath, this.includeMatcher, this.excludeMatcher, this.ignoreFilesBy, this.alwaysIncludeGlobPattern);
    ...
    async loadDocumentFromFilePath(filePath) {
        ...
        try {
            ...
            document = vscode_languageserver_1.TextDocument.create(uri, languageId, 1, text);
        }
        catch (err) {
            timer.error(err);
        }
